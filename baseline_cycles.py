...
def logloss(bag_sizes, real_lps, pred_proba): # Alternative formulation of the logloss (not covered in the memory)
    ...
    loss /= float(np.sum(bag_sizes))
    return -loss
def loglike(bag_sizes, real_lps, pred_proba):
    i=0
    like=0
    for i_b, b in enumerate(bag_sizes):
        lp=real_lps[i_b]
...
for idx_m, m in enumerate(math_model_list):
    ...
    for train, test in kf.split(l_insts):
        ...
        # Note that we also consider the cycle features.
        xtrain=np.zeros([tr_data.embryos.shape[0],
                 tr_data.embryos.shape[1]+tr_data.cycles.shape[1]])
        for e in np.arange(tr_data.num_embryos):
            xtrain[e]=np.concatenate([tr_data.embryos[e],tr_data.cycles[tr_data.embryo_belong_to_cycle[e]]])
        ...
        xtest=np.zeros([ts_data.embryos.shape[0],
                 ts_data.embryos.shape[1]+ts_data.cycles.shape[1]])
        for e in np.arange(ts_data.num_embryos):
            xtest[e]=np.concatenate([ts_data.embryos[e],ts_data.cycles[ts_data.embryo_belong_to_cycle[e]]])
        ...
        embryos_pred_proba = fit_class_embryos.predict_proba(xtest)[:,1]
        ...
        for e in np.arange(ts_data_clean.num_embryos):
            xtest_clean[e]=np.concatenate([ts_data_clean.embryos[e],ts_data_clean.cycles[ts_data_clean.embryo_belong_to_cycle[e]]])
    ...
    with open("results/baseline/aucroc_"+ math_model +"_cycles"+"_hidden"+ ".pickle", 'wb') as metrics_pckl:#f_pckl:
        pickle.dump(l_auc_roc, metrics_pckl, pickle.HIGHEST_PROTOCOL)
    with open("results/baseline/metrics_"+ math_model +"_cycles"+"_hidden"+ ".pickle", 'wb') as metrics_pckl:#f_pckl:
        pickle.dump(l_metrics, metrics_pckl, pickle.HIGHEST_PROTOCOL)
    with open("results/baseline/probabilities_"+ math_model +"_cycles"+"_hidden"+ ".pickle", 'wb') as prob_pckl:#f_pckl:
        pickle.dump(pred_probs, prob_pckl, pickle.HIGHEST_PROTOCOL)
    with open("results/baseline/real_info_"+ math_model +"_cycles"+"_hidden"+ ".pickle", 'wb') as impl_pckl:#f_pckl:
